...
if ( !class_exists('Movedo_Grve_Split_Navigation_Walker') ) {

	class Movedo_Grve_Split_Navigation_Walker extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if( !isset( $this->break_point ) ) {
					...
					if( !empty( $temp_menu ) ) {
						$len = count($temp_menu);
						$split_menu_item_position = movedo_grve_option( 'split_menu_item_position', 'left' );
			...
			if( null != $this->break_point  && ( $this->break_point == $item->menu_order ) ) {
				...
				ob_start();
				do_action( 'movedo_grve_split_logo_before' );
				movedo_grve_print_logo( 'default', 'default' );
				do_action( 'movedo_grve_split_logo_after' );
				$output .= ob_get_clean();

				$output .= '<ul class="grve-menu grve-split-menu-second"><li' . $id . $class_names .'>';
			 } else {
