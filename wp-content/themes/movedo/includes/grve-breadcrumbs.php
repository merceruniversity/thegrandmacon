...
if ( !function_exists( 'movedo_grve_print_breadcrumbs' ) ) {
	function movedo_grve_print_breadcrumbs( $args = array() ) {
		//Yoast Breadcrumbs
...
<?php
			...
			$breadcrumb = new Movedo_GRVE_Breadcrumb( $args );
			return $breadcrumb->trail();
		}
	}
}

class Movedo_GRVE_Breadcrumb {
	...
	public function __construct( $args = array() ) {
		...
		$defaults = array(
			...
			'after'           => '',
			'show_on_front'   => true,
			'home_label'       => esc_html__( 'Home', 'movedo' ),
			'show_title'      => true,
			'post_taxonomy'   => array(
				'post' => 'category',
			),
		...
		$args = apply_filters( 'movedo_grve_breadcrumb_args', $args );
	...
	protected function add_items() {
		...
		if ( is_front_page() ) {
			...
			if ( is_home() ) {
				...
				if ( is_post_type_archive() ) {
					...
					$this->add_rewrite_front_items();
					$year  = sprintf( '%s',  get_the_time( esc_html__( 'Y',  'movedo' ) ) );
					$month = sprintf( '%s', get_the_time( esc_html__( 'F', 'movedo' ) ) );
					$day   = sprintf( '%s',   get_the_time( esc_html__( 'j', 'movedo' ) ) );
					...
					if ( true === $this->args['show_title'] ) {
						$this->items[] = esc_html__( 'Archives', 'movedo' );
					}
				}
			} elseif ( is_search() ) {
				//Search Page
				if ( is_paged() ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_search_link() ), sprintf( esc_html__( 'Search Results for : %s', 'movedo' ), get_search_query() ) );
				} elseif ( true === $this->args['show_title'] ) {
					$this->items[] = sprintf( esc_html__( 'Search Results for : %s', 'movedo' ), get_search_query() );
				}
			} elseif ( is_404() ) {
				//404 Page
				if ( true === $this->args['show_title'] ) {
					$this->items[] = esc_html__( '404 Not Found', 'movedo' );
				}
			}
		}
		...
		if ( is_singular() && 1 < get_query_var( 'page' ) && true === $this->args['show_title'] ) {
			$this->items[] = sprintf( esc_html__( 'Page %s', 'movedo' ), number_format_i18n( absint( get_query_var( 'page' ) ) ) );
		} elseif ( is_paged() && true === $this->args['show_title'] ) {
			$this->items[] = sprintf( esc_html__( 'Page %s', 'movedo' ), number_format_i18n( absint( get_query_var( 'paged' ) ) ) );
		}

		//Filter hook for items
		$this->items = array_unique( apply_filters( 'movedo_grve_breadcrumb_items', $this->items, $this->args ) );
	}
