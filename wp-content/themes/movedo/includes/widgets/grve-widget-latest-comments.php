...
add_action( 'widgets_init', 'movedo_grve_widget_latest_comments' );

function movedo_grve_widget_latest_comments() {
	register_widget( 'Movedo_GRVE_Widget_Latest_Comments' );
}

class Movedo_GRVE_Widget_Latest_Comments extends WP_Widget {

	function __construct() {
		$widget_ops = array(
			'classname' => 'grve-comments',
			'description' => esc_html__( 'A widget that displays latest comments', 'movedo' ),
		);
		$control_ops = array(
			'width' => 300,
			'height' => 400,
		...
		parent::__construct( 'grve-widget-latest-comments', '(Greatives) ' . esc_html__('Latest Comments', 'movedo' ), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
				...
				<li>
					...
					<div class="grve-comment-content">
						<div class="grve-author">
							<?php echo sprintf( _x('%1$s on %2$s', 'Author *on* Post Title', 'movedo'), get_comment_author_link( $comment->comment_ID ), '<a href="' . esc_url( get_comment_link( $comment->comment_ID ) ) . '">' . get_the_title( $comment->comment_post_ID ) . '</a>'); ?>
						</div>
		...
		<?php
			...
			echo esc_html__( 'No Comments Found!', 'movedo' );
		}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'num_of_comments' ) ); ?>"><?php echo esc_html__( 'Number of comments:', 'movedo' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_avatar' ) ); ?>"><?php echo esc_html__( 'Show Avatar:', 'movedo' ); ?></label>
